...
4. Before per-job checks run, the analyzer looks for path@SHA references that
   ...
   than from any pinned ref), and any reviewed contract carrying
   `selectorResultInput` or a privileged caller-permission grant,
...
| Selector failure silently drops required work or routes through an unvalidated variable. | Workloads require the exact selector dependency and a cancellation-safe condition. Ordinary routes use a governed allowlisted literal fallback; every required no-default local call requires the raw output plus exact self-hosted proof and exactly one same-workflow rejection sentinel paired to that selector. The sentinel runs on the approved hosted `failureSentinel` label and is recognized by the declared `failureSentinelMarker` step name under the exact complement, including selector failure; the legacy unroutable `runs-on` shape remains accepted during migration. Fail-closed reporting contracts forward the selector result exactly. | Selector recovery, cancellation, dependency identity, fallback, required-input, same-workflow sentinel pairing, sentinel shape, legacy migration, and result-reporting cases in [`runner-policy.test.mjs`](runner-policy.test.mjs). |
...
| A comment-, review-, or issue-triggered job reaches the fleet carrying untrusted checked-out content by declaring the ancillary-events opt-in. | The policy admits only the exact boolean opt-in inputs (`admits-ancillary-events` and its deprecated alias `admits-comment-events`) set to the literal `true`; any other value, and any unknown input, fails closed. The opt-in relaxes no existing job-level control and changes only which event types the selector will route self-hosted at all. The enforcer still structurally forces a write-capable or credential-bearing ancillary-event job to privileged-hosted unless it holds a local-routing grant or reviewed contract. That classification keys only on `GITHUB_TOKEN` write permissions, named secrets, and credential-minting actions. It has no checkout or PR-head awareness. A permission-read-only opted-in job that checks out untrusted content therefore routes to the fleet on the opt-in alone, with no static backstop; that case is governed solely by human step review of the opted-in job, the trusted no-checkout premise the opt-in declares, exactly as a local-routing grant's job body is trusted. | Boolean opt-in accept, deprecated-alias, non-`true`-value, unregistered-input, owner-scoped v0.8.0 revision, and unapproved-revision cases in [`runner-policy.test.mjs`](runner-policy.test.mjs). |
...
- A job that names the fleet label as a literal has no hosted recovery
  fallback and no failure sentinel. If no runner in the fleet is online it
...
- The ancillary-events opt-in (`admits-ancillary-events`, deprecated alias
  `admits-comment-events`) lets a caller route its `issue_comment`,
  `pull_request_review`, `pull_request_review_comment`, and `issues` jobs onto
  the fleet on its declaration that those jobs check out no PR or issue content.
  The enforcer verifies the input's exact literal value, not that premise; a
  caller that declares the opt-in while actually checking out untrusted content
  would place that content on the fleet. Step review of the opted-in jobs is the
  compensating control, and every job-level control (statically-read-only-or-grant,
  privileged-hosted, container/service, runner-target) still applies unchanged:
  the opt-in widens only the set of event types the selector will route
  self-hosted, never what a routed job may do.
...
- The literal fallback does not make a failed selector healthy; operational
  ...
  fallback cannot silently escalate to a costlier hosted tier. A hosted
  allowlisted fallback preserves scheduling availability at the residual spend
  of that single label; a managed allowlisted fallback spends nothing hosted
  but queues behind the same fleet whose failure may have tripped the selector,
  deferring the fail-closed report until the fleet returns (bounded by GitHub's
  24-hour unmatched-queue limit).
- The selector failure sentinel runs on the approved hosted label and fails
  explicitly through a declared marker step. Its marker name is bounded outside
  every hosted and managed runner label set so it cannot be mistaken for a
  routable target; the one-minute timeout bounds execution on the hosted runner.
