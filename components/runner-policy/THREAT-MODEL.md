...
| A comment-, review-, or issue-triggered job reaches the fleet carrying untrusted checked-out content by declaring the ancillary-events opt-in. | The policy admits only the exact boolean opt-in inputs (`admits-ancillary-events` and its deprecated alias `admits-comment-events`) set to the literal `true`; any other value, and any unknown input, fails closed. The opt-in relaxes no job-level control: a write-capable or checkout-bearing ancillary-event job still reaches the fleet only through a local-routing grant or reviewed contract and otherwise stays privileged-hosted. It changes only which event types the selector will route self-hosted at all. The no-checkout premise the opt-in declares is trusted, compensated by the opted-in job's step review exactly as a local-routing grant is. | Boolean opt-in accept, deprecated-alias, non-`true`-value, unregistered-input, owner-scoped v0.8.0 revision, and unapproved-revision cases in [`runner-policy.test.mjs`](runner-policy.test.mjs). |
