...
| A privileged or credential-bearing job reaches a self-hosted runner. | Local routes require statically read-only permissions unless one immutable runner-input reusable-workflow contract names an exact caller permission and secret boundary. That narrow contract still rejects permission drift and every credential surface outside its exact named secret mapping. Deployment environments, credential-minting actions, job containers, and services remain hosted with an exact categorized exception. | Permission, reusable-caller permission drift, credential-surface, environment, action, container, and service cases in [`runner-policy.test.mjs`](runner-policy.test.mjs). |
