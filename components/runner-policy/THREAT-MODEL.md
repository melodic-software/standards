...
| A managed fleet label is named on a repository that may not route work there, or a repository enrols itself by misdeclaring its own visibility. | A fleet literal is admitted only where `visibility` is `private` and `selfHostedCi` is `true`. Elsewhere it reports `public-self-hosted-routing` or `self-hosted-routing-disabled`, the same rule ids a selector call gets, and `raw-self-hosted-label` still reports the pin. The repository-policy schema forces `selfHostedCi` false when `visibility` is `public`, and GitHub-supplied `CI_REPOSITORY_VISIBILITY` must agree with the checked-in value or analysis fails closed with a configuration error. | Fleet-literal admission, public-repository, non-enrolled, and visibility-evidence cases in [`runner-policy.test.mjs`](runner-policy.test.mjs). |
