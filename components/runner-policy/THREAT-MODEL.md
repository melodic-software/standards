...
4. Before per-job checks run, the analyzer looks for path@SHA references that
   ...
   on either the reviewed basis or the candidate, any job with a
...
| A Dependabot SHA bump leaves the called workflow byte-identical while a commit-relative `uses:` reference inside it — a job-level `./.github/workflows/<file>.yml` nested reusable workflow or a step-level `./…` local action, both resolved by GitHub from the same commit as the file that contains them rather than from any pinned ref — resolves to different content at the new SHA (a nested workflow's runners or secrets, a local action's executable code), and the bump is still auto-approved because the diff only ever fetches the one workflow file being compared. | Any job with a commit-relative `uses:` reference, on either the candidate or any reviewed basis, declines auto-approval unconditionally with a deterministic diagnostic. A nested workflow could in principle be fetched and recursively diffed, but a local action is an arbitrary directory of executable content no single-file fetch can prove unchanged, so both shapes require a human to review the new SHA and add a contract entry. | Repository-local nested-workflow, local-action step, and reviewed-basis local-reference decline cases in [`runner-policy.test.mjs`](runner-policy.test.mjs). |
