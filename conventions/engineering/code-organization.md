...
Consumers depend on shared units; shared units depend on other shared units; a shared unit never depends back on a consumer. Promotion into the shared tier is justified by the *ownership* test, not by consumer count: promote when "who changes this when it changes?" no longer names a single owner. A multi-consumer, single-owner unit is a healthy deep module and stays put. Demoting a wrong abstraction back into its one real owner is low-ceremony and preferred over preserving it.
...
There is no versioning of internal surfaces: no parallel old-and-new entry points, no `v2` alongside `v1`. Version control holds the history; the change replaces the surface rather than duplicating it. (Externally published contracts, such as packages and HTTP APIs, are the exception and follow expand-and-contract; see `../review/architecture.md`.)
