...
The default posture for any code in a consuming repo: write like a senior engineer, with industry-standard practice biased toward simplicity, correctness, and maintainability. These are reasoning-only principles; no tool decides them for you. They set the priors the rest of this catalog refines.
...
- **Timeout-first / resilient by default.** Every outbound call carries a timeout from its first implementation. Add retry with backoff and circuit breakers where a dependency can be slow or flaky. Resilience includes elapsed time: growth stays bounded, expiring material rotates automatically by default, and a restart recovers cleanly. The diff-time bars, including the recorded-reason exception for a deliberate manual rotation path, live in `../review/timebombs.md`.
- **Idempotency by default.** Anything that can be retried must be safe to run more than once with the same effect as once: command handlers, message consumers, endpoints behind a retrying client. Use an explicit idempotency key where the operation is not naturally idempotent.
...
- **Stress-test before presenting.** Attack your own design for edge cases, concurrency, and state leaks, and reject proposals that duplicate a source of truth, over-build, or reinvent an existing mechanism.
...
- **Current state is evidence, never justification.** How code is structured, coupled, or "has always been done" describes what *is*; it does not argue for what *should be*. When a design or boundary's only support is descriptive, such as incumbency, precedent, or existing coupling, re-derive it from first principles and challenge it.
