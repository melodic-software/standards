...
- Everything expensive sits behind that glance, read only when the glance proves insufficient: the full body, the history, the rationale.
...
The layers that load every session are where the discipline pays off most, because their cost recurs: a root instruction file, an always-on rule, a checklist a reviewer opens on every change.
