...
whether a report cites an invocation and its output at all is an existence
check. But a session transcript is not a tracked artifact, so no tool has
anything to gate. The obligation stays with the reader.
...
- **Verifiable now**: a mechanical check exists in this session, such as a count, a
