...
- **CODEOWNERS adoption is deferred.** A CODEOWNERS file only does something once a repository requires reviews from the owners it names; with `required_approving_review_count` held at 0 org-wide (a deliberate single-maintainer choice, not unset drift), CODEOWNERS would be inert. Revisit if that review-count posture changes.
