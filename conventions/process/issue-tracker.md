...
**Structured metadata, not title prefixes.** Set the type with `--type` or the issue form's Types field; set area and other axes with labels. PR titles are unchanged, remaining Conventional Commits enforced by the `pr-title` gate, because squash-merge makes the PR title the default-branch subject line. Issue titles and PR titles serve different layers and follow different rules.
