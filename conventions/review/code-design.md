...
- **Shotgun surgery** — one conceptual change forces the same small edit across many files (add a field, then touch the DTO, mapper, validator, migration, and every handler). A responsibility that should sit in one place is smeared across the codebase; the diff itself is the tell. Important.
