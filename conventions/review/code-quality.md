...
Diff-time checks for local design, parameter and model shape, style, and authoring hygiene: the quality bars automated tooling cannot fully decide. Cross-unit design judgment is owned by [code-design.md](code-design.md): cohesion, coupling, abstraction fit, substitutability, and pattern use. Severity labels (Critical / Important / Suggestion) are defined in [README.md](README.md). Where a check restates a principle owned by an engineering convention, it points there rather than re-explaining it; where a component mechanically enforces a check, it points to that component instead of duplicating the rule.
...
- **Stale or false descriptors**: a behavior change that leaves comments, docs, names, or examples still describing the old behavior, or any edit that introduces a descriptor false about current behavior, including a docs-only or comment-only change. The change is not complete until every descriptor it invalidates is updated. Suggestion for an inline comment; Important where the descriptor is user-facing or operator documentation.
