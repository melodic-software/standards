...
- **Logs to the event stream, not a self-managed sink** — new code that opens, writes, or rotates a log file, or hardcodes a local log path, instead of emitting to standard output for the platform's collection pipeline. Important on a service or container surface; Critical where the sink path will not exist or is not collected in production, silently losing telemetry; Suggestion for a local dev-only script ([the Twelve-Factor App — logs](https://12factor.net/logs)).
