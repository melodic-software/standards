...
- **Logs to the event stream, not a self-managed sink** — new code that opens, writes, or rotates a log file, or hardcodes a local log path, instead of emitting to the process's output stream — standard output, or standard error where standard output is a protocol channel (a stdio-transport server) — for the platform to collect. Important on a service or container surface; Critical where the sink path will not exist or is not collected in production, silently losing telemetry; Suggestion for a local dev-only script ([the Twelve-Factor App — logs](https://12factor.net/logs)).
...
- **Audit entries for high-risk security actions** — a changed hunk that adds or modifies a high-risk security action — authentication, an authorization *failure*, a privilege or role change, or sensitive-data access — with no corresponding audit-log entry. A routine successful authorization check is not an audit event; the failures and privileged grants are. Important; Critical on a compliance-bearing surface ([OWASP application logging vocabulary](https://cheatsheetseries.owasp.org/cheatsheets/Logging_Vocabulary_Cheat_Sheet.html)).
