...
- **Tenant as a correlation tag** — a multi-tenant system's logs, traces, or metrics for a tenant-scoped operation omit the tenant identifier, or an async hop (message, job, event) drops it from the telemetry context, leaving cross-tenant issues impossible to isolate. This is a correlation gap, not a scoping defect — a tenant value that governs data access or an authorization decision is [multi-tenancy.md](multi-tenancy.md)'s concern instead.
