...
- **Audit entries for security-relevant actions** — a changed hunk that adds or modifies a security-relevant action — authentication, an authorization decision, a privilege change, sensitive-data access — with no corresponding audit-log entry. Important; Critical on a compliance-bearing surface ([OWASP application logging vocabulary](https://cheatsheetseries.owasp.org/cheatsheets/Logging_Vocabulary_Cheat_Sheet.html)).
