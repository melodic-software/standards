...
- `--targets OWNER/REPO[,OWNER/REPO...]`, for `matrix` and `plan` only: an exact
  ...
  fleet.
- `--target OWNER/REPO`, for `mappings`, `dest-paths`, and `apply`: the single
...
- `--target-root DIR`, for `apply` only: the local, disposable target checkout
