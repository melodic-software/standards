...
> a stale credential context, a cached value, a mistyped repository. That is
...
- **Storage:** the App's client ID and private key as org-level Actions
  ...
  and **never `ci-workflows`**, so neither secret resolves in a public-repo
...
With `standards` public, an anonymous checkout mounts the same tree, so most
of these no longer depend on the App and are not gated on it: pilot
