...
Four bespoke guards — `comment-hygiene`, `exec-bit`, `machine-specific-paths`,
and `reference-integrity` — gate CI through composite actions in
`melodic-software/ci-workflows`, but had no local lane. Agents and operators
