...
standards/
  ...
  conventions/     # decoupled prose: engineering conventions + code-review criteria (read, not executed; adopted by copy/pointer)
  docs/            # this plan + ADRs
```
...
`conventions/` carries the decoupled prose tooling cannot enforce, re-derived (not ported) from the source corpus and stripped of all agent-harness pointers — split into `engineering/` (the standards a contributor writes by) and `review/` (the criteria a reviewer applies), the latter agnostic-by-default with thin per-stack overlays mirroring the `modules/*` overlays. Curated, not exhaustive: agent-harness machinery (skills, hooks, agent-instruction authoring) and product-specific prose stay in the agent repo; rules already enforced by a `modules/*` config or a hook lane are referenced, not restated (single-owner). Distribution follows consumption — prose is read by humans/agents, not walked by a tool, so it is **adopted by copy or pointer with no runtime coupling** rather than path-pointed like a `modules/*` config. The directory has no fixtures (a reasoning-only standard has no script-assertable pass/fail — that is why it is prose) but dogfoods the repo's own markdown/typos/editorconfig/lychee lanes.
