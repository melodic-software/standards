#!/usr/bin/env bash
# Tests the gitleaks module against the decoupled base config.
#
...
gitleaks dir fixtures/gitleaks/good --config "$config" --no-banner \
  -i fixtures/gitleaks/good >/dev/null 2>&1
rc=$?
